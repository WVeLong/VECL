...
from VECL.paths import BIOCLINICAL_BERT_PATH
...
class BertEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if not self.bert_type:
            self.bert_type = BIOCLINICAL_BERT_PATH
        self.model = AutoModel.from_pretrained(self.bert_type)
    ...
    def forward(self, ids, attn_mask, token_type):
        ...
        outputs = self.model(ids, attention_mask=attn_mask, token_type_ids=token_type, output_hidden_states=True, return_dict=True)

        if self.last_n_layers > 1:
            if hasattr(outputs, "hidden_states"):
                all_embeddings = outputs.hidden_states
            elif len(outputs) > 3 and outputs[3] is not None:
                all_embeddings = outputs[3]
            else:
                all_embeddings = outputs[2]
            if all_embeddings is None:
                raise RuntimeError("Text encoder did not return hidden states. Check the transformers model configuration.")
