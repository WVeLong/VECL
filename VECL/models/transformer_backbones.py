...
def mrm_vit_b16(pretrained_path, **kwargs):
    model = MRM(
        patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = load_weight(model, pretrained_path) ## train from M3AE
    return model


def vit_b_16(pretrained_path):
    model = mrm_vit_b16(pretrained_path)
