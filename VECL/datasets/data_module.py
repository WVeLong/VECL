...
class FinetuneDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.dataset = pretraining_dataset.FinetuneDataset
        self.collate_fn = pretraining_dataset.Classify_collate_fn

    def train_dataloader(self):
        transform = builder.build_transformation(self.cfg, "train")
        dataset = self.dataset(cfg=self.cfg, transform=transform)
        return DataLoader(
            dataset,
            pin_memory=True,
            drop_last=True,
